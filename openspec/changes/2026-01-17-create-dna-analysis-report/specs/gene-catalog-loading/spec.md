# Spec: Gene Catalog Loading

## Overview

This specification defines the capability to load and parse the gene catalog from `hidden/important-genes-2/` directory structure.
...
- [`gene-information-management`](../../../specs/gene-information-management/spec.md) - Defines how gene information is structured and managed
- [`dna-data-parsing`](../dna-data-parsing/spec.md) - Defines how DNA data is parsed for matching
- [`variant-matching`](../variant-matching/spec.md) - Defines how loaded genes are matched against DNA data

## Notes

- The gene catalog is organized into 10 category directories under `hidden/important-genes-2/`
- Each gene file follows the YAML schema defined in the gene catalog creation proposal
- The catalog loader should handle the large number of genes (393) efficiently
- Category folder names are lowercase and hyphenated for consistency
- The loader should be case-insensitive for rsid lookups
- Cross-category gene references should be handled via the notes field in gene files
- The loader should support future additions to the gene catalog without code changes
